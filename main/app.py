...
with colA:
    ...
    if os.path.exists("reports/roc.png"):
        st.image("reports/roc.png", caption="ROC")
    if os.path.exists("reports/calibration.png"):
        st.image("reports/calibration.png", caption="Reliability diagram (calibration)")
                ...
                with st.expander("Misclassification gallery (top FPs & FNs with XAI)"):
                    ...
                    if len(sel_fp)==0 and len(sel_fn)==0:
                        ...
                        def render_case(idx, title):
                            ...
                            with cols[1]: st.image(gc_img, caption=f"Grad-CAM (outside-lung {out_lung:.0%})")
                            with cols[2]: st.image(ig_img, caption="Integrated Gradients")
...
with colB:
    ...
    if target_img is None:
        ...
        st.image(pil.resize((img_size, img_size)), caption=f"Original{' — GT: '+src_label if src_label else ''}")
        ...
        if st.button("Generate explanations (Grad-CAM + IG)"):
            ...
            ig = ig_map(model, x)
            c1, c2 = st.columns(2)
            with c1: st.image(overlay_heatmap(pil, gc, size=img_size), caption="Grad-CAM")
            with c2: st.image(overlay_heatmap(pil, ig, size=img_size), caption="Integrated Gradients")
